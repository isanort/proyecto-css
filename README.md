# 🖌️ Where is my CSS?

This is a CSS project for the FemCoders Bootcamp.\
➡️ Web development- Frontend

## 💡 Project Description

Oh no! We accidentally deleted the CSS code from our login page.

![image info](./assets/css-model.webp)

Luckily, we have recovered the ```<body>``` of our log in page.\
We are going to structure the CSS code missing.

```
<div class="card">
   <form>
      <h2 class="title"> Log in</h2>
      <p class="subtitle">Don't have an account? <a href="#"> sign Up</a></p>

      <div class="social-login">
         <button class="google-btn">
            <img alt="Google" src="data:image/svg+xml;base64," />
            <p class="btn-text">Sign in with Google</p>
         </button>
         <button class="fb-btn">
         <img alt="FB" src="data:image/svg+xml;base64," />
         </button>        
      </div>

      <p class="or"><span>or</span></p>

      <div class="email-login">
         <label for="email"><b>Email</b></label>
         <input type="text" placeholder="Enter Email" name="uname" required>
         <label for="psw"><b>Password</b></label>
         <input type="password" placeholder="Enter Password" name="psw" required>
      </div>
      <button class="cta-btn">Log In</button>
      <a class="forget-pass" href="#">Forgot password?</a>
   </form>
</div>
```
### 📌 Main task

Continuing with the same team from the "Where is my HTML?" project, we will design and apply CSS styles to the existing Log In form, following a provided HTML structure. 

### 📂 Working space

We will work in a CSS file that will be saved inside a specific folder.

### ℹ️ Detailed directions

We will use CSS3 properties and tags to style the form, including:
- Positioning
- Colors
- Typography
- Spacing\
This will be achieved while ensuring a responsive and visually appealing design.
> ⚠️ The HTML file must not be modified. Only CSS can be added

### 🛠️ Tools

- CSS3 &emsp; <img alt="html icon" src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"> 
- A text editor like Visual Studio Code &emsp; <img alt="vscode icon" src="https://img.shields.io/badge/VSCode-0078D4?style=for-the-badge&logo=visual%20studio%20code&logoColor=white">

### 📊 Evaluation

The project will be evaluated based on:

- Proper application of CSS styles
- CSS code organisation
- Ability to maintain a design in-line with the given HTML structure


## 🎯 Project Goal

In this **pair-programming activity**, the main goal is to **create the HTML structure for a Log In form**, applying different levels of understanding of web layout.  
Moreover, you’ll practice pushing the project to a GitHub repository, while applying **Git** and **Gitflow** best practices.

## 📷 Preview

![image info](./assets/preview-proyecto-css.png)

## 🚀 Deploy

In progress

## 🤝  Team Members

- [Bruna Sonda](https://github.com/brunasonda)  
- [Iris Sánchez](https://github.com/isanort)